...
from legodnn.common.manager.model_manager.common_pose_estimation_model_manager_v2 import CommonPoseEstimationModelManager
from cv_task.datasets.pose_estimation import mmpose_build_dataloader
from cv_task.pose_estimation.mmpose_models.legodnn_configs import get_deepose_res18_mpii_256_256_310e_128b_config
from cv_task.pose_estimation.mmpose_tools import mmpose_init_model
from mmcv.parallel import MMDataParallel

if __name__=='__main__':
    cv_task = 'pose_estimation'
    dataset_name = 'coco'
    model_name = 'mmpose_deeppose_res50_coco_256x192'
    # compress_layer_max_ratio = 0.25
    compress_layer_max_ratio = 0.125
    device = 'cuda'
    model_input_size = (1, 3, 256, 192)
    train_batch_size = 64
    test_batch_size = 64
    ...
    test_sample_num = 100
    model_config = get_deepose_res18_mpii_256_256_310e_128b_config()
    ...
    if teacher_pt_file is not None:
        teacher_detector = mmpose_init_model(config=model_config, checkpoint=None, mode='mmdet_test', device=device)
        ...
        teacher_detector = mmpose_init_model(config=model_config, checkpoint=checkpoint, mode='mmdet_test', device=device)
