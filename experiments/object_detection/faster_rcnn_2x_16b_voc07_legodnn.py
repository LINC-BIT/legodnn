...
from cv_task.object_detection.mmdet_models.legodnn_configs import get_faster_rcnn_r50_fpn_2x_b16_voc2007_config
from cv_task.object_detection.mmdet_tools import mmdet_init_model, train_detector_by_config

if __name__=='__main__':
    cv_task = 'object_detection'
    dataset_name = 'voc07'
    model_name = 'faster_rcnn_r50_fpn_b16'
    epoch_num = '2x'
    device = 'cuda'
    method = 'legodnn'
    ...
    trained_blocks_dir_path = root_path + '/trained_time_test'
    descendant_models_dir_path = root_path + '/descendant'
    block_training_max_epoch = 2
    test_sample_num = 100
    model_config = get_faster_rcnn_r50_fpn_2x_b16_voc2007_config('train')
    ...
    if teacher_pt_file is not None:
        teacher_detector = mmdet_init_model(config=model_config, checkpoint=None, mode='mmdet_train', device=device)
        raw_teacher = torch.load(teacher_pt_file).to(device)
        for name, module in teacher_detector.named_modules():
            if len(list(module.children()))>0:
                continue
            else:
                set_module(teacher_detector, name, copy.deepcopy(get_module(raw_teacher, name)))
    else:
        teacher_detector = mmdet_init_model(config=model_config, checkpoint=checkpoint, mode='mmdet_train', device=device)
