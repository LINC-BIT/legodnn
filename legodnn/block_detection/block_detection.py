...
class BlockDetection:
    def __init__(self, graph: LegoDNNGraph, min_compress_num=2, max_compress_num=4) -> None:
        ...
        self.blocks_start_node_name_hook: List[str] = [] # 记录每个块的输入应该钩原始模型中层的名字
        self.blocks_start_node_hook_input_or_ouput = []  # 记录钩输入还是输出，如果为0则钩输入，如果为1则钩输出
        self.blocks_start_node_hook_index = [] # 记录钩出的结果在hook的输入输出list的位置
        
        self.blocks_end_node_name_hook: List[str] = []  # 记录每个块的输出应该钩原始模型中层的名字
        self.blocks_end_node_hook_input_or_ouput = []  # 记录钩输入还是输出，如果为0则钩输入，如果为1则钩输出
        self.blocks_end_node_hook_index = [] # 记录钩出的结果在hook的输入输出list的位置
        # 子图约束3的条件
    ...
    def _adjust_is_block(self, block: List[int]):
        ...
        start_input_compressed_layers = self._find_block_all_paths_first_compressed_layer_name_from_input(block)
        start_output_compressed_layers = self._find_block_all_paths_first_compressed_layer_name_from_output(block)
    ...
    def _adjust_block_hook_node_input_and_output(self, block: List[int], start_node_is_placeholder: bool):
        ...
        out = (start_node_name_hook, start_node_hook_input_or_ouput, start_node_hook_index, end_node_name_hook, end_node_hook_input_or_output, end_node_hook_index)
    ...
    def _find_block_all_paths_first_compressed_layer_name_from_input(self, block: List[int]):
        start_node = self.graph.order_to_node[block[0]]
        node_queue = Queue()
    ...
    def _find_block_all_paths_first_compressed_layer_name_from_output(self, block: List[int]):
        end_node = self.graph.order_to_node[block[-1]]
        node_queue = Queue()
        compressed_layers_name = []
    ...
    def detection_all_block(self):
        ...
        while(start_order < node_sum):
            ...
            while node_order<=node_sum:
                ...
                if is_subgraph:
                    # 判断是否满足块的子图约束其它条件：1、所有的可重用层必须在同一个块内；2、满足块可压缩要求(至少存在一条路径，该路径上有至少一个可压缩层) NOTE：目前的实现属于残缺版；3、满足块中可压缩层数要求
                ...
                if is_subgraph and is_block:
                    # exit(0)
                    # 判断开始节点是否为占位符，占位符的意思是该节点是上一个块的输入或者是func操作
                    ...
                    if block_able_train: 
                        ...
                        self.blocks_no_compressed_layers.append(self._find_block_all_paths_first_compressed_layer_name_from_output(block))
    ...
    def print_blocks(self):
        ...
        for i, block in enumerate(self.blocks):
            ...
            if self.blocks_start_node_hook_input_or_ouput[i]==0:
                print("当前块的输入：{}的输入, 索引为{}".format(self.blocks_start_node_name_hook[i], self.blocks_start_node_hook_index[i]))
            else:
                print("当前块的输入：{}的输出, 索引为{}".format(self.blocks_start_node_name_hook[i], self.blocks_start_node_hook_index[i]))

            if self.blocks_end_node_hook_input_or_ouput[i]==0:
                print("当前块的输出：{}的输入, 索引为{}".format(self.blocks_end_node_name_hook[i], self.blocks_end_node_hook_index[i]))
            else:
                print("当前块的输出：{}的输出, 索引为{}".format(self.blocks_end_node_name_hook[i], self.blocks_end_node_hook_index[i]))
