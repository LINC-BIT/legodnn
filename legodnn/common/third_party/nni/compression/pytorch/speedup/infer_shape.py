...
infer_from_inshape = {
    ...
    'ConvTranspose2d': lambda module_masks, mask: conv2dt_inshape(module_masks, mask),
    ...
    'EltwiseAdd': lambda module_mask, mask: eltwise_add_inshape(module_mask, mask),
    'TwoAdd': lambda module_masks, mask: add_inshape(module_masks, mask),
    
    # yolov3
    'ZeroPad2d': lambda module_masks, mask: maxpool2d_inshape(module_masks, mask),
    
    'Tanh': lambda module_masks, mask: relu_inshape(module_masks, mask)
}
...
def eltwise_add_inshape(module_masks, mask):
    """
    Inference the output mask of the add operation from the
    input mask.
    """
...
def conv2dt_inshape(module_masks, mask):
    """
    Shape change of input tensor does not affect the shape of its output tensor
    Parameters
    ----------
