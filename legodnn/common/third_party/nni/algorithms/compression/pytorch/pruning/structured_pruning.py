...
import logging
import math
from zedl.dl.common.model import get_module
from zedl.third_party.nni.common.graph_utils import TorchModuleGraph
import numpy as np
import torch
import random
from .weight_masker import WeightMasker

# from util.model import LayerActivation
...
class StructuredWeightMasker(WeightMasker):
    ...
    def _get_current_state(self, sparsity, wrapper, wrapper_idx=None):
        ...
        if isinstance(wrapper.module, torch.nn.Conv2d):
            num_total = weight.size(0)
        elif isinstance(wrapper.module, torch.nn.ConvTranspose2d):
...
class TRRPrunerMasker(StructuredWeightMasker):
    """
    A filter pruner via Triplet Response Residual (TRR).
    B. Fang, X. Zeng, and M. Zhang,
    “NestDNN: Resource-aware multi-tenant on-device deep learning for continuous mobile vision,”
    in Proc. ACM Mobicom, 2018, pp. 115–127.

    Implemented by queyu, 2020/11/13
    """

    def __init__(self, model, pruner, preserve_round=1, dependency_aware=False, train_loader=None, device='cuda'):
        super().__init__(model, pruner, preserve_round, dependency_aware)

        assert train_loader is not None, 'TRR needs the training dataset'
    ...
    def get_mask(self, base_mask, weight, num_prune, wrapper, wrapper_idx, channel_masks=None):
        trrs = self.get_channel_sum(wrapper, wrapper_idx)
        if channel_masks is not None:
            # if we need to mask some channels in advance
            trrs = trrs * channel_masks

        threshold = torch.topk(
            trrs, num_prune, largest=False)[0].max()
        mask_weight = torch.gt(trrs, threshold)[
                      :, None, None, None].expand_as(weight).type_as(weight)
        mask_bias = torch.gt(trrs, threshold).type_as(
            weight).detach() if base_mask['bias_mask'] is not None else None

        return {'weight_mask': mask_weight.detach(), 'bias_mask': mask_bias}

    def get_channel_sum(self, wrapper, wrapper_idx):
        # add hook in the target module
        module_activation = LayerActivation(wrapper.module, '', 'cpu')
        self.model.eval()

        filter_num = wrapper.module.weight.data.size()[0]
        trrs = torch.zeros(filter_num)
        ...
        with torch.no_grad():
            for data, target in self.train_loader:
                cur_data_num += data.size()[0]
                # if cur_data_num > data_num * 0.2:
                #     continue

                data, target = data.to(self.device), target.to(self.device)
                self.model(data)

                # construct {anc, pos, neg} from data and target
                # compute TRR from data in module_activation.output
                triplets = self._constrcut_trr_triplets(data, target)
                triplets_num += len(triplets)
                trrs += self._compute_trr(triplets, module_activation.output)

        module_activation.remove()
        return trrs

    def _constrcut_trr_triplets(self, data, target):
        # return the indexes, not the data

        # randomly select the anc in data
        anc = random.randint(0, data.size()[0] - 1)
        anc_label = target[anc]

        triplets = []
        poses = []
        negs = []

        # find the pos, which has the same class with anc
        for i, (item, label) in enumerate(zip(data, target)):
            if i == anc:
                continue
            # pos
            if label == anc_label:
        ...
        # min_len = min(len(poses), len(negs))
        # poses = poses[0: min_len + 1]
        # negs = negs[0: min_len + 1]
    ...
    def _compute_trr(self, triplets, feature_map):
        def l2_norm(data):
            return (data ** 2).sum(dim=(1, 2))

        trr = torch.zeros(feature_map.size()[1])
        for triplet in triplets:
            anc, pos, neg = triplet
            a = l2_norm(feature_map[anc] - feature_map[neg])
            b = l2_norm(feature_map[anc] - feature_map[pos])
            trr_item = a - b
            trr += trr_item
    ...
    def calc_contributions(self):
        ...
        for wrapper in self.pruner.get_modules_wrapper():
            if isinstance(wrapper.module, torch.nn.Conv2d):
                ...
                filters = wrapper.module.weight.size(1)
                contribution = (
                    wrapper.module.weight*wrapper.module.weight.grad).data.pow(2).view(filters, -1).sum(dim=1)
                if wrapper.contribution is None:
                    wrapper.contribution = contribution
...
class ActivationFilterPrunerMasker(StructuredWeightMasker):
    def __init__(self, model, pruner, statistics_batch_num=1, activation='relu'):
        ...
        assert activation in ['relu', 'relu6', None]
        if activation == 'relu':
    ...
    def _add_activation_collector(self, pruner):
        def collector(collected_activation):
            def hook(module_, input_, output):
                if pruner.activation is not None:
...
class HRankPrunerMasker(ActivationFilterPrunerMasker):
    ...
    def _calc_hrank(self, activations, wrapper):
        if isinstance(wrapper.module, torch.nn.Conv2d):
            activations = torch.cat(activations, 0)
            hrank_stats = torch.zeros((activations.size()[0], activations.size()[1]))
            
            for activation_index in range(activations.size()[0]):
                activation = activations[activation_index]
                
                for feature_map_index in range(activation.size()[0]):
                    feature_map = activation[feature_map_index]
                    
                    rank = torch.matrix_rank(feature_map)
                    hrank_stats[activation_index][feature_map_index] = rank
            
            # print(hrank_stats[0], hrank_stats[1], hrank_stats[2])
            hrank_stats = torch.mean(hrank_stats, dim=0)
            # print(hrank_stats)
            # raise NotImplementedError()
            return hrank_stats
    ...
    def get_channel_sum(self, wrapper, wrapper_idx):
        ...
        if len(activations) < self.statistics_batch_num:
            # collected activations is not enough
            return None
        return self._calc_hrank(activations, wrapper).to(wrapper.module.weight.device)
    
    
# implemented by queyu, 2021/6/10
class PFPMasker(ActivationFilterPrunerMasker):
    ...
    def _calc_filter_sen(self, activations, wrapper):
        if isinstance(wrapper.module, torch.nn.Conv2d):
            activations = torch.cat(activations, 0)
            ...
            while True:
                successors = self.pruner.graph.find_successors(cur_layer_name)
                if len(successors) == 0:
                    break

                next_layer_name = successors[0]
                next_layer = get_module(self.model, next_layer_name)
                if isinstance(next_layer, torch.nn.Conv2d):
                    next_conv_layer = next_layer
                    break
                
                cur_layer_name = next_layer_name

            self.pruner._wrap_model()
            
            next_conv_layer_weight = next_conv_layer.weight.data
            unfold_next_conv_layer_weight = next_conv_layer_weight.view((next_conv_layer_weight.size()[0], -1))
            unfold_activations = torch.nn.functional.unfold(
                activations,
            ...
            ).to(next_conv_layer_weight.device)
            
            res = torch.zeros((activations.size()[0], next_conv_layer_weight.size()[1]))
            
            
            print(unfold_next_conv_layer_weight.size(), unfold_activations.size())

            for i in range(unfold_next_conv_layer_weight.size()[0]):
                t = unfold_next_conv_layer_weight[i].unsqueeze(0).unsqueeze(-1) * unfold_activations
                t = torch.max(t, dim=-1)[0]
                t = t.view((activations.size()[0], next_conv_layer_weight.size()[1], -1))
                t = torch.max(t, dim=-1)[0]
                t = t.cpu()
                res = torch.max(res, t)
            ...
            res = torch.zeros((activations.size()[0], next_conv_layer_weight.size()[0]))
            
            for i in range(unfold_next_conv_layer_weight.size()[1]):
                print(unfold_next_conv_layer_weight.size(), unfold_activations.size())
                t = unfold_next_conv_layer_weight[:, i].unsqueeze(1).unsqueeze(-1) * unfold_activations
                t = torch.max(t, dim=-1)[0]
                t = t.view((activations.size()[0], next_conv_layer_weight.size()[0], -1))
                t = torch.max(t, dim=-1)[0]
                t = t.cpu()
                res = torch.max(res, t)
    ...
    def get_channel_sum(self, wrapper, wrapper_idx):
        ...
        if len(activations) < self.statistics_batch_num:
            # collected activations is not enough
            return None
        return self._calc_filter_sen(activations, wrapper).to(wrapper.module.weight.device)