...
class SingleStageObjectDeteDetectionManager:
    def __init__(self, model_graph: LegoDNNGraph, detection_mode='whole', whole_compress_num_range=(4,6), detection_cfg={}) -> None:
        ...
        if self.detection_mode == 'whole':
            ...
            if detection_cfg.get('bbox_head'):
                self.component_types.append('bbox_head')
                bbox_head_graph = self.graph.get_subgraph('bbox_head')
                bbox_head_compress_num_range = detection_cfg.get('bbox_head_compress_num_range')
                bbox_head_mode = detection_cfg.get('bbox_head_mode')
                bbox_head_block_detection = BaseBlockDetection(bbox_head_graph, min_compress_num=bbox_head_compress_num_range[0], 
                                                        max_compress_num=bbox_head_compress_num_range[1], mode=bbox_head_mode)
                # neck_block_detection.specialed_output_nodes = self._find_all_specialed_output_nodes_by_order(neck_graph, model_graph)
                self.component_block_detections.update({'bbox_head': bbox_head_block_detection})  
    ...
    def detection_all_blocks_of_components(self):
        if self.detection_mode == 'whole':
            ...
            for component_type in self.component_types:
                ...
                elif component_type == 'bbox_head':
                    self.component_block_detections['bbox_head'].detection_all_block()
        ...
        if self.detection_mode == 'whole':
            ...
            for component_type in self.component_types:
                if component_type == 'backbone':
                    ...
                    print('bbox_head部分, 共有{}个块'.format(len(self.component_block_detections['bbox_head'].blocks)))
                    self.component_block_detections['bbox_head'].print_blocks()    
