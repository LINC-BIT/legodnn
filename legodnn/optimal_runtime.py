...
class OptimalRuntime:
    ...
    def infer(self, batch_data):
        ...
        if self._device == 'cpu':
            with torch.no_grad():
                ...
                self._model_manager.dummy_forward_to_gen_mid_data(model, self._model_input_size)
            ...
            with torch.no_grad():
                ...
                model = model.to(self._device)
                # self._model_manager.forward_to_gen_mid_data(model, batch_data, self._device)
                self._model_manager.dummy_forward_to_gen_mid_data(model, self._model_input_size, self._device)
    ...
    def update_model(self, cur_max_infer_time, cur_max_model_size):
        ...
        def solve(cur_max_infer_time, cur_max_model_size):
            ...
            act_acc_drop = apply(variables, self._acc_drops) # 精度下降
