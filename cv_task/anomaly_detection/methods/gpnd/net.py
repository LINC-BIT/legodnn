...
class NewEncoder(nn.Module):
    ...
    def forward(self, input):
        x = self.relu1(self.conv1_1(input))
...
class GPND(torch.nn.Module):
    def __init__(self, zsize, nc) -> None:
        super(GPND, self).__init__()

        self.e = Encoder(zsize, channels=nc)
        self.g = Generator(zsize, channels=nc)
...
class NewGPND(torch.nn.Module):
    def __init__(self, zsize, nc) -> None:
        super(NewGPND, self).__init__()

        self.e = NewEncoder(zsize, channels=nc)
        self.g = NewGenerator(zsize, channels=nc)
