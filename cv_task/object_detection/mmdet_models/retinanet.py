...
def retinanet_r50_fpn(device='cuda'):
    config = os.path.join(configs_root_path, 'retinanet/retinanet_r50_fpn_1x_coco.py')
    checkpoint = os.path.join(pretrained_root_path, 'retinanet_r50_fpn_1x_coco_20200130-c2398f9e.pth')
    detector = init_detector(config, checkpoint, device=device) 
    return detector

if __name__=='__main__':
    model = retinanet_r50_fpn()
    print(model)