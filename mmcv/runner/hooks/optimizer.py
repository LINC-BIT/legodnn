...
class OptimizerHook(Hook):
    ...
    def after_train_iter(self, runner):
        if hasattr(runner, 'method') and runner.method is not None:
            method = getattr(runner, 'method')
            if method == 'usnet':
                # print('US-Net: after train iter')
                ...
                # print('NestDNN: after train iter')
                # runner.outputs['loss'].backward()
                if self.grad_clip is not None:
                    grad_norm = self.clip_grads(runner.model.parameters())
                    if grad_norm is not None:
            ...
            print('before backward: {}'.format(datetime.now()))
            runner.outputs['loss'].backward()
            print('after backward: {}'.format(datetime.now()))
            if self.grad_clip is not None:
                grad_norm = self.clip_grads(runner.model.parameters())
                if grad_norm is not None:
                    # Add grad norm to the logger
                    runner.log_buffer.update({'grad_norm': float(grad_norm)},
                                            runner.outputs['num_samples'])
            print('before step: {}'.format(datetime.now()))
            runner.optimizer.step()
    ...
    def before_train_iter(self, runner):
         if hasattr(runner, 'method') and runner.method is not None:
            method = getattr(runner, 'method')
            if method == 'usnet':
                # print('US-Net: before train iter')
                runner.optimizer.zero_grad()
            elif method == 'cgnet':
                # print('CGNet: before train iter')
                runner.optimizer.zero_grad()
            elif method == 'fn3':
                # print('CGNet: before train iter')
                runner.optimizer.zero_grad()
