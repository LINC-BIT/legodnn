...
**Module details**
- **BlockManager**: the block manager moudle integrates `block extrator`, `descendant block generator`, and `block swicher`. The `block extrator` is mainly responsible for extracting the original blocks from the convolution layer of the original model, and the `descendant block generator` is responsible for pruning the original blocks generated by the block extrator to generate multiple sparsity descendant blocks. `Block swicher` is responsible for replacing blocks in the model at run time with optimal blocks selected according to the optimization algorithm. This project has implemented automatic block extraction for a variety of models through AutoBlockManager.。
- **BlockRetrainer**：the `block retrainer` moudle use the intermediate data between layers generated by the original data on the original model as training data, and the orgiginal blocks as the optimization target to train the descendant blocks, improving the accuracy of the descendant blocks. This training can be fast because it uses only intermediate data between blocks, avoiding the unnecessary computations on other parts of the model, and these intermediate data can be used to train multiple descendant blocks generated from the same original block in parallel.
- **BlockProfile**：the `block profile` directly obtains a block’s memory footprint according to its model size and considers k representative model sparsities/sizes in the profiling of each descendant block and calculate the block’s accuracy loss as the average accuracy loss of these k profiles.
- **LatencyProfile**：the `latency profile` moudle is used to analyze the percentage of latency reduction for blocks on edge devices. Latency is obtained by simulating inference on each block directly on the edge device, and the percentage of delay reduction for each block is calculated using (latency of the original block - latency of the current derived block)/latency of the original block.
- **ScailingOptimizer**：the `scaling optimizer` moudle is used to updates blocks in real time for optimization. By formalizing the block selection as an integer linear programming optimization problem and solving it, the block corresponding to the maximum accuracy under specified delay and memory conditions can be selected at runtime.
