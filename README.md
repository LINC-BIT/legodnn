...
- **目标检测**是功能是检测出图像中目标（如人、狗、车等）对应的检测框的坐标以及目标的识别。主流的目标检测网络可以分为三个部分：Backbone+Neck+检测头。如图（c）所示是基于主流Neck结构FPN的Yolov3网络，其Backbone对应图中的Conv和四个Stage，也就是Resnet18中的Root和四个Stage。检测头就是常见的线性预测层。

- **行为识别**是识别出视频片段中目标的行为，如挥手说话等。如图（d）所示为经典的行为识别双流网络。模型被分为空间卷积网络和时间卷积网络，两者都是做分类任务且均使用图像分类网络。
 
 - **异常检测**是检测视频或者图像中的异常情况，主要被应用于欺诈检测，入侵检测，生态灾难预警等。异常检测网络主要分为两种：基于Self-training的模型（分类）和基于GAN的模型（重构）。如图（e1）和图（e2）所示。基于Self-traing的模型主要是通过Resnet进行特征提取，全连接层用于分类预测。基于GAN的模型是简单对称的AutoEncoder模型。
