...
## 简介
 目前使用比较广泛的主要有六种视觉类DNN应用，包括**图像分类、目标检测、语义分割、异常检测、动作识别和语义识别。**
...
- **目标检测**是功能是检测出图像中目标（如人、狗、车等）对应的检测框的坐标以及目标的识别。主流的目标检测网络可以分为三个部分：Backbone+Neck+检测头。如图（b）所示是基于主流Neck结构FPN的Fast RCNN网络，其Backbone对应图中的Conv和四个Stage，也就是Resnet18中的Root和四个Stage。检测头就是常见的线性预测层。

	
 - **语义分割**是对图像中的每一个像素点进行分类，目前广泛应用于医学图像和无人驾驶等场景中。语义分割网络通常是一种编码器-解码器结构。如图（c）所示为经典语义分割网络FCN解码器结构。与目标检测网络一样，编码器对应于图像分类网络，用于提取特征，解码器各有不同。
	
 
 - **异常检测**是检测数据中心的异常情况，主要被应用于欺诈检测，入侵检测，生态灾难预警等。异常检测网络主要分为两种：基于Self-training的模型（分类）和基于GAN的模型（重构）。如图（e1）和图（e2）所示。基于Self-traing的模型主要是通过Resnet进行特征提取，全连接层用于分类预测。基于GAN的模型是简单对称的AutoEncoder模型。
	行为识别是识别出视频片段中目标的行为，如挥手说话等。如图（d）所示为经典的行为识别双流网络。模型被分为空间卷积网络和时间卷积网络，两者都是做分类任务且均使用图像分类网络。
...
LegoDNN（[文章](https://dl.acm.org/doi/abs/10.1145/3447993.3483249)）是一个针对模型缩放问题的轻量级、块粒度、可伸缩的解决方案，通过将原始DNN模型中抽取块，生成稀疏派生快，然后对这些块进行再训练。通过组合这些块，扩大原始模型的伸缩选项。并且在运行时，通过算法对块的选择进行了优化。以Resnet18为例，如下图所示。 本项目是一个对LegoDNN的基于PyTorch的实现，支持将以上六种主流应用场景下的深度神经网络转换为LegoDNN，从而增加大量的缩放选项，在边缘端进行动态缩放以适应设备资源的变化。
